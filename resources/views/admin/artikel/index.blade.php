...
<div class="card">
	...
	<button type="button" class="btn rounded" data-toggle="modal" data-target="#tambah" style="max-width: 170px;margin-left:10px;background-color:#6059f6;color:white" >Input Data Baru <i class="icon-newspaper"></i></button>

	<table id="tabelArtikel" class="table datatable-basic table-bordered table-striped table-hover">
		<thead>
			<tr>
				<th>Nomor</th>
				<th>Judul Artikel</th>
	...
	<div class="modal-dialog modal-lg" style="border-radius: 10px">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="mb-0">Tambahkan Artikel Baru</h6>		
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>

			<div class="modal-body">
                <form action="{{Route('tambahArtikel')}}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
					...
					<div class="form-group">
						...
						<div>
							<label>Konten :</label>
							<textarea name="content" type="text" class="form-control" rows="20" placeholder="Masukkan pokok bahasan..."></textarea>
						</div>
					</div>

					<div class="input-group hdtuto control-group lst increment" >
						<input type="file" name="upload[]" class="myfrm form-control" multiple accept="image/x-png,image/gif,image/jpeg">
...
<div id="edit{{$data->id_artikel}}" class="modal fade" tabindex="-1">
	<div class="modal-dialog modal-full modal-dialog-scrollable" style="border-radius: 10px; overflow: scroll !important;">
		<div class="modal-content">
			<div class="modal-header">
			...
			<div class="modal-body">
                <form action="{{Route('editArtikel', $data->id_artikel)}}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}
					...
					<div class="form-group">
						<label>Konten Bahasan :</label>
						<textarea class="form-control" value="{{$data->content}}" rows="40" name="content">{{$data->content}}</textarea>
					</div>
	...
	<script>
		$(document).ready(function() {
			...
			$("#tabelArtikel").DataTable({
				...
				"dataSrc": "data",
				"columns": [
					{data: 'id_artikel', name: 'nomor'},
					{data: 'judul', name: 'judul'},
					{data: 'created_at', name: 'created_at'},
					{data: 'id_artikel', 'targets' : [2], 'render': function(data){
