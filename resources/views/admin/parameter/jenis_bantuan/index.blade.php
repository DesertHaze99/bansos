...
<div class="card">
	...
	<table id="tabelJenisBantuan" class="table datatable-basic table-bordered table-striped table-hover">
		<thead>
			<tr>
				<th>Nomor</th>
				<th>Nama Bantuan</th>
	...
	<div class="modal-dialog" style="border-radius: 10px">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="mb-0">Tambahkan Data Jenis Bantuan Baru</h6>		
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>

			<div class="modal-body">
                <form action="{{Route('tambahJenisBantuan')}}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}

					<div class="form-group">
						<div >
							<label>Nama Bantuan :</label>
							<input type="text" class="form-control" placeholder="Masukkan nama bantuan..." name="bantuan">
						</div>
					</div>
...
<div id="edit{{$data->id_bantuan}}" class="modal fade" tabindex="-1">
	<div class="modal-dialog" style="border-radius: 10px">
		<div class="modal-content">
			<div class="modal-header">
				<h6 class="mb-0">Ubah Data Jenis Bantuan</h6>		
				<button type="button" class="close" data-dismiss="modal">&times;</button>
			</div>

			<div class="modal-body">
                <form action="{{Route('editJenisBantuan', $data->id_bantuan)}}" method="POST" enctype="multipart/form-data">
                    {{ csrf_field() }}

					<div class="form-group row">
						<div class="col-lg-6">
							<label>Nama Jenis Bantuan :</label>
							<input type="text" class="form-control" value="{{$data->bantuan}}"  name="bantuan">
						</div>
					</div>
...
@section('script')
	<script>
		$(document).ready(function() {
			...
			document.getElementById("jenisBantuan").classList.add("nav-item-open");
            ...
            $("#tabelJenisBantuan").DataTable({
                ...
                "serverSide": true,
                "ajax": {
						'url':"{{ url('jenisBantuanAJAX') }}",
                        'headers':"{{ csrf_token() }}"
						},
                "order": ['0', 'desc'],
                "dataSrc": "data",
                "columns": [
                    {data: 'id_bantuan', name: 'nomor'},
					{data: 'bantuan', name: 'bantuan'},
					{data: 'created_at', name: 'tahun'},
					{data: 'id_bantuan', 'targets' : [2], 'render': function(data){
